    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="analytics" className="space-y-6">
          ...
          {/* My Top Properties & Monthly Performance */}
          <div className="grid gap-6 lg:grid-cols-2">
            {/* My Most Sold Properties */}
            <Card className="shadow-soft">
              <CardHeader>
              ...
              <CardContent className="space-y-3">
                {[
                  { name: "Lekki Gardens Phase 2", sales: 12, revenue: "₦18.5M", growth: "+25%" },
                  { name: "Abuja Estate Phase 1", sales: 9, revenue: "₦14.2M", growth: "+18%" },
                  { name: "Ibeju Commercial Land", sales: 8, revenue: "₦12.8M", growth: "+32%" },
                  { name: "Victoria Island Apartments", sales: 7, revenue: "₦11.3M", growth: "+15%" },
                  { name: "Epe Farmland Plots", sales: 5, revenue: "₦6.2M", growth: "+8%" },
                ].map((property, idx) => (
                  <div key={property.name} className="p-3 rounded-lg bg-muted/30">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {property.sales} sales • {property.revenue}
                        </p>
                      </div>
            ...
            {/* My Monthly Performance Trend */}
            <Card className="shadow-soft">
              <CardHeader>
                <CardTitle>My Monthly Performance</CardTitle>
                <CardDescription>Your sales and revenue trend over time</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {[
                  { month: "January", sales: 8, revenue: "₦12.5M", target: 85 },
                  { month: "February", sales: 12, revenue: "₦18.3M", target: 95 },
                  { month: "March", sales: 15, revenue: "₦21.8M", target: 110 },
                  { month: "April", sales: 10, revenue: "₦14.9M", target: 75 },
                ].map((month) => (
                  <div key={month.month} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">{month.month}</span>
                      <span className="text-sm text-muted-foreground">
                        {month.sales} sales • {month.revenue}
                      </span>
                    </div>
                    <div className="relative h-2 bg-muted rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-primary to-yellow-500 transition-all"
                        style={{ width: `${month.target}%` }}
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">Target Achievement</span>
                      <span className={`text-xs font-semibold ${
                        month.target >= 100 ? 'text-green-600 dark:text-green-400' :
                        month.target >= 75 ? 'text-blue-600 dark:text-blue-400' :
